...
/// A transaction included in a flashblock, with its raw bytes for DA calculation.
#[derive(Debug, Clone)]
...
pub struct FlashblockInclusion {
    ...
    /// Transactions included in this flashblock, with raw bytes for DA calculation.
...
impl ResourceAnnotator {
    ...
    fn handle_flashblock_event(&mut self, event: FlashblockInclusion) {
...

        {
            ...
            for included_tx in &event.transactions {
                if let Some(mut tx) = self.pending_transactions.shift_remove(&included_tx.tx_hash) {
                    // Compute DA bytes from the raw transaction
                    tx.data_availability_bytes = flz_compress_len(&included_tx.raw_tx) as u64;
...
mod tests {
        ...
        FlashblockInclusion {
            ...
            transactions: hashes
                ...
                .map(|h| {
                    ...
                    IncludedTransaction {
                        tx_hash: B256::new(hash_bytes),
                        // Use a simple test payload for DA calculation
                        raw_tx: Bytes::from(vec![0u8; 100]),
                    }
                })
                .collect(),
        }
    ...
    #[tokio::test]
    async fn pending_transactions_evict_oldest_when_limit_exceeded() {
        let cache = Arc::new(RwLock::new(MeteringCache::new(10)));
        let (_tx_sender, tx_rx) = mpsc::unbounded_channel();
        let (_fb_sender, fb_rx) = mpsc::unbounded_channel();
        let (_cmd_sender, cmd_rx) = mpsc::unbounded_channel();
        ...
        // Add MAX_PENDING_TRANSACTIONS transactions
        for i in 0..super::MAX_PENDING_TRANSACTIONS {
            annotator.handle_tx_event(test_tx(i as u64, i as u64));
        }

        assert_eq!(annotator.pending_transactions.len(), super::MAX_PENDING_TRANSACTIONS);

        // Get the hash of the first (oldest) transaction
        let oldest_hash = {
            let mut hash_bytes = [0u8; 32];
            hash_bytes[24..].copy_from_slice(&0u64.to_be_bytes());
            B256::new(hash_bytes)
        };
        assert!(annotator.pending_transactions.contains_key(&oldest_hash));

        // Add one more transaction (should trigger eviction)
        let new_tx_id = super::MAX_PENDING_TRANSACTIONS as u64;
        annotator.handle_tx_event(test_tx(new_tx_id, new_tx_id));
        ...
        let new_hash = {
            let mut hash_bytes = [0u8; 32];
            hash_bytes[24..].copy_from_slice(&new_tx_id.to_be_bytes());
            B256::new(hash_bytes)
        };
        assert!(annotator.pending_transactions.contains_key(&new_hash));
    }
}
