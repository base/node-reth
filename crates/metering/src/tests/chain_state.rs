...
async fn meter_bundle_simulation_reflects_pending_state() -> Result<()> {
    ...
    harness
        .build_block_from_transactions(vec![deploy_bytes])
        .await?;
    ...
    let set_call = Counter::setNumberCall {
    ...
    harness
        .build_block_from_transactions(vec![set_bytes])
        .await?;
...
async fn meter_bundle_errors_when_beacon_root_missing() -> Result<()> {
    ...
    let flash_call = Counter::setNumberCall {
        newNumber: U256::from(99u64),
    };
    ...
    let flashblock = build_single_flashblock(
        ...
        vec![(
            flash_bytes.clone(),
            Some((flash_envelope.tx_hash(), receipt.clone())),
        )],
    );
    ...
    assert!(
        ...
        "expected flashblock to populate pending state"
    );
    assert_eq!(
...
fn envelope_from_signed(tx: TransactionSigned) -> (OpTxEnvelope, Bytes) {
    let op_signed = OpTransactionSigned::Eip1559(
        tx.as_eip1559()
            .expect("transaction should be EIP-1559")
            .clone(),
    );
